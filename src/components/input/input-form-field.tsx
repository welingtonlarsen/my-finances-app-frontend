...
export function InputFormField<T extends FieldValues>({form, name, label, type}: TInputForm<T>) {
  const icon = useMemo(() => {
    if (type === 'number') return <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400">$</span>
    return null
  }, [])

  const handleOnChangeNumberInput = (event: ChangeEvent<HTMLInputElement>, field: ControllerRenderProps<T, Path<T>>) => {
  ...
  const getInput = useCallback((field: ControllerRenderProps<T, Path<T>>) => {
    if(type === 'number')
      return <Input className="pl-6" type='number' {...field}
